 ...
 * ```
 *
 * 무급 휴게가 근무 전체와 같으면 새 정산식의 차감 분모가 0이 되므로 막는다.
 */
describe('breakMinutesRule', () => {
  it('무급 휴게가 근무 시간과 정확히 같으면 거부한다', () => {
    const result = breakMinutesRule('09:00', '18:00', 540, false)
    expect(result.valid).toBe(false)
    expect(result.message).toContain('무급 휴게시간')
  })

  it('유급 휴게가 근무 시간과 정확히 같으면 통과한다', () => {
    expect(breakMinutesRule('09:00', '18:00', 540, true).valid).toBe(true)
  })
  ...
  it('자정을 넘기는 근무도 실제 길이로 잰다', () => {
    ...
    expect(breakMinutesRule('22:00', '06:00', 479).valid).toBe(true)
