<script setup>
import { ArrowUpRight, Info, Plus } from 'lucide-vue-next'
    ...
    <div class="label-row">
      ...
      <div v-if="infoOpen" :id="infoId" class="balance-info" role="note">
        ...
        <p class="balance-info-body">
          사업장 운영에 바로 쓸 수 있는 금액이에요.<br />
          근무 계약으로 예치(에스크로)한 금액은<br />
          별도로 예치중에 표시되며,<br />
          이 잔액에는 포함되지 않아요.
        </p>
      </div>
...
/* 잔액·충전/출금 버튼 위로 겹쳐 뜨되(예치중 팝오버와 같은 오버레이 방식), 오른쪽에
   붙여 내용 길이만큼만 차지한다 — left를 stretch하면 텍스트보다 훨씬 넓어져 오른쪽에
   빈 공간이 남는다. z-index 로 아래 콘텐츠 위에 그려진다. */
.balance-info {
  position: absolute;
  z-index: 1;
  top: 0;
