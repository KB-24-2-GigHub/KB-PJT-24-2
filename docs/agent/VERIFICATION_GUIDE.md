...
| `WorkCaseControllerTest`, `WorkCaseServiceImplTest`, `WorkCaseServiceDatabaseIntegrationTest`                  | R0~R3 | 2-1~2-3·2-6, WORK-001~006·008                     | DRAFT CRUD, 야간 16시간·휴게 검증, Version 증가, PENDING 철회, 소유권, 동시 수정, `dailyWage` 직접 저장 | 현재 Work 생성·수정 계약과 정합                         | 유지 |
| `InvitationIssue*`, `InvitationQueryServiceImplTest`, `InvitationLifecycleDatabaseIntegrationTest`            | R1~R3 | 2-5·3-2, INVITE-001~004, BADGE-003                | 발급/기존 Link/재발급, Token 상태·만료·Version, 보건증 요구와 OWNER 0~3단계 Badge                 | 현재 초대 조회 계약과 정합                                  | 유지 |
...
| `SettlementCalculationPolicyTest`, `SettlementPayout*`, `NoShowRefund*`, `CheckOutMissingRefund*`, `SettlementCalculationSnapshotSchemaDatabaseIntegrationTest` | R1~R3 | 5A-6~8·5B-5~7·5C-3, SETTLE-001~006, WALLET-005·006 | 분 단위 올림·10원 절사 Snapshot, 수동/자동 경합, 지급·차액/전액 환불 원장, 분쟁 보류, Replay·Rollback·동시성 | 현재 정산·환불 계약과 정합 | 유지 |
| `AttendanceScan*`, `AttendanceLifecycleDatabaseIntegrationTest`                                                | R1~R3 | 5A-2·4, 5B-2, 5C-1~2, ATT-002~006                | QR·위치·경계 감사, 조기 퇴근 확인, Terminal 전이 상호배타·멱등과 최초 계산 Snapshot               | 현재 근태 API·Scheduler 계약과 정합                         | 유지 |
| `Document*ControllerTest`, `Document*ServiceTest`, `DocumentAccessAuditSchemaDatabaseIntegrationTest`         | R1~R3 | 3-6, DOC-001~012                                  | 목록·쓰기 응답, 표시 파일명, 공유·접근자 격리, 파일 Header·Checksum, 비공개 Key와 감사 행         | 현재 문서 계약과 정합                                       | 유지 |
| Frontend `*.spec.js`와 `*.test.js` 전체                                                                        | R0~R2 | P0 화면·Route·Service 현재 동작                   | 요청 Shape, Redirect, 중복 클릭, 비금융 경과 참고, 정산 Snapshot·Badge·상태 표시                 | Production build와 실제 API 활성 여부는 별도 판정           | 유지 |
...
| Work 생성·수정·삭제      | 생성 201, 수정·삭제 204, 소유권/상태 4xx, `dailyWage`와 야간·휴게 검증                         | 현재 저장·응답 계약과 정합                                                                    |
| 초대 발급·조회           | 최초 201/기존 200, Bearer Token, 약정 일급·보건증 요구·OWNER Badge                            | 현재 조회 필드와 인증 경계에 정합                                                              |
...
| 정상·지각·조퇴 정산      | 200, OWNER 권한, 저장 Snapshot 비례 지급·차액 환불, 자동 경합, 분쟁 보류, 정확한 Replay       | 현재 SETTLE-001~006 계약과 정합                                                               |
| 노쇼·퇴근 누락 환불      | 200, 별도 OWNER Operation·Claim, 전액 환불 Snapshot, 분쟁 차단, WORKER 무변경                 | 현재 환불 계약과 정합                                                                          |
| 문서 목록·쓰기·파일      | 200/201, 닫힌 Item Shape·표시 파일명, Header·Checksum·권한·감사·404                           | 현재 문서 계약과 정합                                                                          |
