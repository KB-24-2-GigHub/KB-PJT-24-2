...
6. For any task that interprets product scope, priority, demo success, or `P0`/`P1`/`P2`/`Deferred`, read `docs/specs/MVP_SCOPE.md` completely before the active canonical contract. Treat it as the immutable top-level product-goal input; use the related canonical requirements, API, decisions, and traceability documents for stable IDs and formal contracts. Then load only the current issue's related `draft` Patch and the detailed domain, runbook, schema, API, or testing documents that the task actually touches. Do not preload the documentation tree.
