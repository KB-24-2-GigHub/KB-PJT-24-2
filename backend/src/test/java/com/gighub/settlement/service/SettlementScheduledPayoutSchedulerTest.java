...
class SettlementScheduledPayoutSchedulerTest {
    ...
    void paysEachCandidateAndSkipsThoseAlreadyClaimed() {
        ...
        when(payoutService.attemptPayout(2L, NOW)).thenReturn(null);

        scheduler().runOnce();

        verify(payoutService).attemptPayout(1L, NOW);
        verify(payoutService).attemptPayout(2L, NOW);
    ...
    void isolatesOneCandidateFailureAndContinuesWithTheRest() {
        ...
        when(payoutService.attemptPayout(2L, NOW))
    ...
    void doesNotProcessAnyCandidateWhenBatchLookupFails() {
        ...
        verify(payoutService, times(0)).attemptPayout(anyLong(), any());
    }
