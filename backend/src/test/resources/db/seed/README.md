...
- `video-03-check-out`: 김성실·이수면은 퇴근 가능한 `IN_PROGRESS`, 박잠수는 `NO_SHOW`입니다.
  박잠수 정산에는 노쇼 환불 승인에 필요한 지급 0원·사장 환불 전액 Snapshot이 포함됩니다.
- `video-04-three-years`: 3개 사업장, 누적 근무, 3/2/1 근로자 배지, 오늘 3호점 일정,
  보존기간 안의 과거 계약 64건과 실제 PDF 128개를 만듭니다. 이후 실제 API 수락으로 지점별
  최신 계약 3건을 더하고, 3년 보존기간이 지난 계약 1건은 파기 완료 상태로 남깁니다.

`functional`과 `video-04-three-years`는 SQL 뒤 실제 API를 호출하므로 Tomcat을 먼저 실행해야
합니다. `video-04-three-years`는 운영 계약서 Renderer로 과거 PDF를 일괄 생성하므로 다른
시나리오보다 실행 시간이 더 걸립니다.

모든 시나리오는 독립적이며 앞 시나리오의 데이터를 이어받지 않습니다. 이수면의 30분 지각
기록과 실제 임금 비례 차감·부분 지급·차액 환불이 포함됩니다. `functional` 시나리오의 기존 이수면 근무도 같은 자동 노쇼 마감이 있으므로
...
사장 계좌는 시나리오별 금액을 사용하고, 나머지 Mock 은행계좌 22개는 모두 잔액과 출금 가능액을
`5,000,000원`으로 생성합니다.

1호점 위치는 `서울 광진구 능동로 195-16`, 위도 `37.5481384`, 경도
`127.0733972`입니다. 일반 화면에서 등록한 사업장 반경은 기존 `100m`이고,
`demo-*.sql`이 만드는 시연 전용 사업장만 촬영 편의를 위해 `999999m`를 사용합니다.
이 확대 반경과 합성 은행계좌는 시연 환경 전용이며 실제 사용자 데이터가 있는 환경에는 적용하지 않습니다.
