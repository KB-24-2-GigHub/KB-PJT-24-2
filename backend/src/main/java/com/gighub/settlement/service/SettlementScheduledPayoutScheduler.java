 ...
 * 잡습니다.</p>
 *
 * <p>시간 판정은 앱 서버 시계가 아니라 {@link SettlementMapper#currentDatabaseTime}이 돌려주는
 * MySQL {@code NOW(6)}을 그대로 쓴다(SETTLE-003). 앱 서버와 DB 서버 시계가 어긋나 있어도 지급
 * 자격 판단이 DB 한 기준으로만 이뤄지게 하기 위함이다.</p>
 *
...
public class SettlementScheduledPayoutScheduler {
    ...
    public void runOnce() {
        LocalDateTime now;
        List<ScheduledPayoutCandidate> candidates;
        try {
            now = settlementMapper.currentDatabaseTime();
            candidates = settlementMapper.findScheduledPayoutCandidates(
                    now, properties.getBatchSize());
        } catch (RuntimeException failure) {
            // 시각 조회·후보 조회 자체가 실패해도 다음 실행 주기에서 다시 시도하면 되므로 예외를 삼킨다.
            log.warn("Settlement 예정 자동 지급 후보 조회에 실패했습니다.", failure);
            return;
        }
        processCandidates(candidates, now);
    }

    private void processCandidates(List<ScheduledPayoutCandidate> candidates, LocalDateTime now) {
        int paid = 0;
        int skipped = 0;
        int failed = 0;
        for (ScheduledPayoutCandidate candidate : candidates) {
            switch (processOne(candidate, now)) {
                case PAID -> paid++;
        ...
        if (paid > 0 || failed > 0) {
            log.info(
                    ...
                    candidates.size(), paid, skipped, failed);
        }
    }

    private Outcome processOne(ScheduledPayoutCandidate candidate, LocalDateTime now) {
        try {
            SettlementResult result = payoutService.attemptPayout(
                    candidate.getSettlementId(), candidate.getWorkCaseId(), now);
            return result == null ? Outcome.SKIPPED : Outcome.PAID;
        } catch (RuntimeException failure) {
            return recordFailure(candidate.getSettlementId(), failure, now);
        }
    }
