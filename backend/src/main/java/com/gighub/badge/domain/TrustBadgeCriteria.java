...
public final class TrustBadgeCriteria {
    ...
    /**
     * 다음 등급에 필요한 정상 비율 문턱입니다. 건수는 채웠는데 비율이 부족해 등급이 오르지
     * 못한 경우, API 응답의 {@code criterionDesc}가 이 값으로 부족한 조건을 안내합니다.
     */
    private static int nextThresholdPercent(int level) {
        if (level >= 3) {
            return 0;
        }
